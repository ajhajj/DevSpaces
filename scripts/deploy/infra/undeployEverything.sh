...
clear
bannerMsg "Undeploying OpenShift Dev Spaces" | tee ${INSTALL_LOGFILE}
. ./deployDevSpaces.sh 2>&1 | tee -a ${INSTALL_LOGFILE}
pipedErrChck "Undeploying OpenShift Dev Spaces"

SETTLING_TIME=5
bannerMsg "Undeploying new admin user" | tee -a ${INSTALL_LOGFILE}
. ./undeployAdminUser.sh 2>&1 | tee -a ${INSTALL_LOGFILE}
pipedErrChck "Undeploying new admin user"
